...
class Supro_WooCommerce {
	...
	function hooks() {
		...
		// Change shop columns
		if ( supro_get_option( 'catalog_layout' ) == 'masonry' ) {
		...
		//add_action( 'woocommerce_single_product_summary', array( $this, 'product_header_summary' ), 10 ); /zrx
	...
	function product_class( $classes, $class = '', $post_id = '' ) {
		...
		if ( ! is_single( $post_id ) ) {
			...
			if ( ! is_search() ) {
				...
				if ( $catalog_layout == 'masonry-content' ) {
	...
	function product_cat_class( $classes, $class = '', $category = '' ) {
		...
		if ( $catalog_layout == 'masonry-content' ) {
			if ( $woocommerce_loop && isset( $woocommerce_loop['loop'] ) ) {
				$index = intval( $woocommerce_loop['loop'] ) % 3;
				if ( $index == 1 ) {
